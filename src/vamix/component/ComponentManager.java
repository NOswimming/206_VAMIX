...
/**
 * A singleton, holds references to each of the current components
 * and handles creation of components.
 */
public class ComponentManager {
	...
	/**
	 * @param dir: the absolute path to the file.
	...
	public int addAudio(String dir) throws FileNotFoundException {
		...
		for(Audio a: audioList) {
			if(a.getFile() == f) return a.getID();
		}
		
		Audio a = new Audio(f);
		a.setID(getID());
		audioList.add(a);//Add the audio object to the list of objects to be used
		return a.getID();
	...
	/**
	 * 
	 * @param ID: the unique ID of the Audio component
