...
import vamix.function.OverlayAudioFunction;
...
/**
 * The GUI component for Overlaying audio on to a video.
 * It is shown by pressing the Overlay Audio button on the MainPanel.
 * It calls the OverlayAudioFunction to run the avconv process.
 * 
 * @see #MainPanel #OverlayAudioFunction
 * 
 * @author Callum Fitt-Simpson
 */
public class OverlayAudioDialog extends JDialog implements ActionListener {
	...
	private OverlayAudioFunction overlayAudio;
	...
	public OverlayAudioDialog() {
		...
		JLabel lbl_Filename = new JLabel("  Output File Name");
		...
		JLabel lbl_AudioFile = new JLabel("Audio file");
		panel_Directory.add(lbl_AudioFile);

		textField_AudioFile = new JTextField();
		textField_AudioFile.setColumns(20);
		textField_AudioFile.setEditable(false);
		panel_Directory.add(textField_AudioFile);
		...
		btn_FileChooser.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int returnVal = fileChooser.showOpenDialog(OverlayAudioDialog.this);
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					textField_AudioFile.setText(fileChooser.getSelectedFile()
							.getAbsolutePath());
				}
			}
		});
		...
		btn_StripAudio = new JButton("Overlay Audio");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btn_StripAudio) {
			...
			String audioFile = textField_AudioFile.getText();
			if (name == null ||  name.equals("") || audioFile == null ||  audioFile.equals("")) {
				JOptionPane.showMessageDialog(this,
						"Please complete the filename and select an audio file.");
				return;
			}
			String directory = "/home/";
			File wd = ComponentManager.getInstance().getWorkingDirectory();
			if (wd != null) directory = wd.getAbsolutePath();
			overlayAudio = new OverlayAudioFunction(this, name, directory, audioFile);
			overlayAudio.execute();
		}
